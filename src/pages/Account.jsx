...
const Account = () => {
    ...
    // Ambil semua fungsi dari useUsers termasuk uploadAvatar
    const { 
        users, 
        loading, 
        error, 
    ...
    const handleEditUser = async (userId, userData) => {
        try {
            // Cek apakah userData adalah FormData (untuk avatar) atau object biasa
            const isFormData = userData instanceof FormData;
            
            let result;
            
            if (isFormData) {
                // Jika FormData, berarti ini request upload avatar
                console.log('Uploading avatar for user:', userId);
                result = await uploadAvatar(userId, userData.get('avatar')); // Ambil file dari FormData
            } else {
                // Jika object biasa, update data user
            ...
            if (selectedUser && selectedUser.id === userId) {
                if (isFormData) {
                    // Jika upload avatar, refresh data user untuk mendapatkan avatar baru
                    if (refreshUsers) {
                        await refreshUsers();
                        // Cari user terbaru di daftar users
